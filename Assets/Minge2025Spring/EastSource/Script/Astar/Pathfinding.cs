...
{ 
    ...
    public List<PathNode> FindPath(BaseEnemyWalkable startCell, BaseEnemyWalkable distinationCell)
    {
        graph = new PathGraph();
        //初期化
        BaseEnemyWalkable currentCell;
        ...
        openQueue.Enqueue(startCell.gameObject.name, Math.Abs(Vector3.Distance(startCell.gameObject.transform.position, distinationCell.gameObject.transform.position)));
        ...
        while (openQueue.Count > 0)
        {
            ...
            //Goalに隣接していれば終了
            foreach (BaseEnemyWalkable connectEnemyWalkableCell in distinationCell.AroundCells)
            {
                if (currentCell == connectEnemyWalkableCell)
                {
                    foreach (var pathNodeValue in graph.GetNodeDict().Values)
            ...
            //Goalに隣接していなければ周囲のノードを調べてOpenQueueに追加
            OpenNeighborNode(currentId, openQueue, closedList, distinationCell);
        }
        Debug.Log("Stupid");
        return null;
    }
    ...
    private void OpenNeighborNode(string parentId, PriorityQueue<string, float> openQueue, List<string> closedList, BaseEnemyWalkable distinationCell)
    {
        List<PathEdge> currentEdgeList = graph.GetPathEdge(parentId);
        foreach (PathEdge currentEdge in currentEdgeList)
        {
            ...
            //Totalのコストを計算してみて今までの方がスコアが低ければ
            if (currentNode.UpdateFCost(graph.GetPathNode(parentId), distinationCell))
            {
                openQueue.Enqueue(currentNode.NodeId, currentNode.FCost);
            }
        }
